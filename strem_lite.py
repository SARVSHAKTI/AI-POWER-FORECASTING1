...
# 📌 Prophet Model
def prophet_model():
    df_prophet = pd.DataFrame({"ds": pd.date_range(start="2023-01-01", periods=len(y_true)), "y": y_true})
    model = Prophet()
    model.fit(df_prophet)
    future = model.make_future_dataframe(periods=0)
    forecast = model.predict(future)
    return calculate_metrics(y_true, forecast["yhat"])
...
for model_name, model_func in models.items():
    try:
        ...
        best_model = "Verification"  # This can be set based on additional validation
